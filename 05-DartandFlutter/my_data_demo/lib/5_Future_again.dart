...
class _MyWidgetState extends State<MyWidget> {
  ...
  void initState() {
    ...
    // 일반 함수에서 비동기 함수를 호출 할 때는 then을 사용
    _loadData().then((value) {
      debugPrint('then');
      debugPrint('데이터 불러오기 완료');
    });
    debugPrint('initState 다른 작업 1');
    debugPrint('initState 다른 작업 2');
    debugPrint('initState 다른 작업 3');
    debugPrint('initState 다른 작업 4');
    debugPrint('initState 다른 작업 5');
  }
      ...
      body: FutureBuilder<void>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            return Center(
              child: Text(
                '비동기 함수 실습 1: $_data (sanpshot.hasData: ${snapshot.hasData})',
              ),
            );
          }
          return Center(
